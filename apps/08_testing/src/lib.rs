...
//! # 08_testing —— GPUI 测试框架与最佳实践（桌面 / Android 同构）
//!
//! 工程结构与 07 一致：本文件（`lib.rs`）是条件编译入口，Android 走 `android_main`
//! （产出 cdylib `.so` 给 NativeActivity 加载），桌面走 `run()` + `gpui_platform::
//! application()`，二者共用 `open_window()` 装配计数器视图。桌面二进制 `src/main.rs`
//! 只是一个调用 `run()` 的薄壳。
//!
//! 本例同时是官方 `examples/testing.rs` 的搬运：演示 GPUI 的测试设施（见文件末尾
//! `#[cfg(test)] mod tests`）。注意测试只在 `cargo test` 时参与编译，不影响 Android
//! 构建。运行应用：`cargo run -p testing_08`；运行测试：`cargo test -p testing_08
...
/// 打开窗口并装配 `Counter`（桌面 / Android 共用）。
/// Android 上窗口即整个屏幕（`window_bounds: None`）；桌面端给一个固定居中窗口。
fn open_window(cx: &mut App) -> gpui::WindowHandle<Counter> {
...
mod android_entry {
    ...
    pub fn android_main(app: android_activity::AndroidApp) {
        ...
        std::panic::set_hook(Box::new(|info| {
            log::error!("testing_08 panic: {info}");
        }));
