//! 字符串模糊匹配：命令面板、语言选择器、补全列表等都用它。
//!
//! 与 zed `crates/fuzzy_nucleo/src/strings.rs` 同 API（`StringMatch` 的字段、
//! `match_strings` / `match_strings_async` 的签名与语义一致）。
...
/// 一个待匹配的字符串。`char_bag` 在构造时算好，供预筛复用。
#[derive(Clone, Debug)]
...
/// 一个匹配结果。`positions` 是**字节**偏移，喂给高亮渲染用。
#[derive(Clone, Debug)]
...
impl StringMatch {
    /// 把连续的 `positions` 合并成区间，直接用于高亮（如 `HighlightedLabel`）。
...
/// 并行分段版：候选很多时用（按 CPU 数切成段，各自一个 `nucleo::Matcher`）。
///
/// `cancel_flag` 置 true 后，各段会在下一个候选处提前退出并返回空结果。
...
/// 空查询（或全空白）：不打分，按原顺序返回前 `max_results` 个。
...
mod tests {
    ...
    #[test]
    fn test_ranges_merges_adjacent_positions() {
        let cs = candidates(&["hello"]);
        let results = match_strings(&cs, "hel", Case::Ignore, LengthPenalty::Off, 10);
        let ranges: Vec<_> = results[0].ranges().collect();
