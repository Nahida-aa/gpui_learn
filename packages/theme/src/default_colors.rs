...
impl ThemeColors {
    ...
    pub fn light() -> Self {
        ...
        Self {
            ...
            // ---- 本仓库增补的字段 ----
            selection_background: blue().light().step_3().alpha(0.35),
            editor_cursor: neutral().light().step_12(),
            link: blue().light().step_11(),
        }
    }
    ...
    pub fn dark() -> Self {
        ...
        Self {
            ...
            vim_helix_select_foreground: system.transparent,
            // ---- 本仓库增补的字段 ----
            selection_background: blue().dark().step_3().alpha(0.35),
            editor_cursor: neutral().dark().step_12(),
            link: blue().dark().step_11(),
        }
    }
}
...
// ---------------------------------------------------------------------------
//
// 上面 `ThemeColors::{light,dark}` 是 zed 的默认灰阶配色；这一节是**本仓库
// 内置主题实际使用的配色**（Catppuccin Mocha / Latte）。
//
// 为什么放在这里而不是单独的 `builtin.rs`：与 zed 的文件划分一致——
// zed 把「内置配色」放在 `default_colors.rs`（色板 + `ThemeColors::{light,dark}`），
// 没有单独的 builtin 文件。我们的内置主题换了配色（Catppuccin 而非灰阶），
// 但角色相同，故同样落在本文件。
//
...
use gpui::{FontStyle, FontWeight, HighlightStyle, WindowBackgroundAppearance, hsla};

use crate::state::{Appearance, Theme, ThemeStyles};
use crate::styles::{AccentColors, PlayerColors, StatusColors, SyntaxTheme};
