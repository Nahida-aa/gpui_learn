//! KeyBinding：按键提示控件（对齐 zed `crates/ui/src/components/keybinding.rs`）。
//!
//! 它把一个 `Action` 或一组 `KeybindingKeystroke` 渲染成人类可读的按键提示
//! （Mac 上是图形键帽 `⌘⇧S`，Linux/Windows 上是 `Ctrl+Shift+S` 文本）。
//!
...
use gpui::{
    ...
    Modifiers, SharedString, Window, relative,
};
