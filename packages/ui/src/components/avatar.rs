//! 用户头像及其状态指示器（对齐 zed `crates/ui/src/components/avatar.rs`）。
//!
//! 出处：zed `crates/ui/src/components/avatar.rs`（GPL-3.0-or-later）。
//! 与 zed 的差异见文件内 `// 与 zed 的差异` 注释（主要是 `Icon` / `Img` 的
//! 尺寸与取色 API 不同）。

use documented::Documented;
use gpui::{AnyElement, AnyView, Hsla, ImageSource, Img, IntoElement, Styled, img, rems};

use crate::prelude::*;
use crate::{IconSize, rems_from_px};
...
impl Avatar {
    ...
    /// 与 zed 的差异：`Avatar::render` 需要知道图标尺寸来算容器，
    /// zed 用 `IconSize`；我们的 `Icon::size` 收 `Pixels`，
    /// 这里把 `IconSize::Small` 换算成 px 后用 `px()` 表达。
    fn fallback_icon_size() -> Pixels {
        px(14.)
    }
}
...
impl RenderOnce for AvatarAudioStatusIndicator {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        // 与 zed 的差异：zed 用 `IconSize::Indicator.rems()` 直接拿 rems；
        // 我们的 `Icon::size` 收 `Pixels`，统一换算成 px 再布局。
        let icon_rems = IconSize::Indicator.rems();
        let icon_px = icon_rems * window.rem_size();
        let padding_x = px(4.);
        ...
        v_flex()
            ...
            .children(vec![
                example_group(vec![
                    ...
                    .description(
                        "Can be used to create visual space by setting the border color to match the background, \
                        which creates the appearance of a gap around the avatar.",
                    ),
                ]),
                example_group_with_title(
                    ...
                    vec![
                        ...
                        .description(
                            "Indicates that both the collaborator's mic and audio are muted.",
                        ),
                        single_example(
                            "Availability: Free",
                            Avatar::new(example_avatar)
                        ...
                        .description(
                            "Indicates that the person is free, usually meaning they are not in a call.",
                        ),
                        single_example(
                            "Availability: Busy",
                            Avatar::new(example_avatar)
                        ...
                        .description(
                            "Indicates that the person is busy, usually meaning they are in a channel or direct call.",
                        ),
                    ],
                ),
            ])
