//! `InputField`：单行输入字段（对齐 zed `crates/ui_input/src/input_field.rs`）。
//!
//! 它包一层单行编辑器（经 [`crate::ErasedEditor`] 运行时注入），再补上标签、
//! 占位符、前导图标、masked（密码/API key）、校验错误提示这些表单属性。
//!
//! 出处：zed `crates/ui_input/src/input_field.rs`（GPL-3.0-or-later）。
//! 适配点见文件内 `// 与 zed 的差异` 注释。

use aa_gpui_kit_component::{example_group, single_example};
use aa_gpui_kit_ui::Tooltip;
use aa_gpui_kit_ui::prelude::*;
use aa_gpui_kit_ui::IconSize;
...
pub struct InputField {
    ...
    ///
    /// Its position is determined by the layout of the field itself.
...
impl InputField {
    /// 构造一个输入字段。
    ///
    /// **前置条件**：必须先调用过 `editor::bind_input_keys(cx)`（它会注册
    /// [`crate::ERASED_EDITOR_FACTORY`]），否则这里会 panic。见 crate 顶部文档。
...
impl Render for InputField {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // 与 zed 的差异：zed 的 `Icon` / `IconButton` 收 `IconSize`、`.color()` 收语义
        // `Color`；我们这两个控件收 `Pixels` / `Hsla`（见 aa_gpui_kit_ui 的 icon.rs 与
        // button/mod.rs），所以这里在渲染期用 rem_size 换算。
