...
// ── 从生成时注入的常量 ──────────────────────────────────────────────────────
val rustLibName = "{RUST_LIB_NAME}"          // cargo cdylib 的 [lib] name → lib<RUST_LIB_NAME>.so
val cargoPackage = "{CARGO_PACKAGE}"         // workspace 里的 cargo 包名，用于 cargo ndk -p
val abiList = listOf({TARGETS})              // 目标 ABI 列表（默认 arm64-v8a + x86_64）
